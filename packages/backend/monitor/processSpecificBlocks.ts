        ...
        if (events.length > 0) {
          ...
          for (const event of events) {
            await addEventToRSS(
              ...
              event.proposalLink,
              event.timestamp,
              convertBigIntToString(event.args) // Convert BigInt values to strings
            );
          }
        }
      } catch (error) {
...
export const processSpecificBlocks = async (config: NetworkConfig) => {
  ...
  if (config.chainId === 324) {
    await processAddressBlocks(zkSyncAddressBlocks, config);
  }
  
  if (config.chainId === 1) {
    await processAddressBlocks(ethereumAddressBlocks, config);
  }
